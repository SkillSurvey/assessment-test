...
const indexWrappers = document.querySelectorAll(".index-wrapper");
const btnToResults = document.querySelector(".btn-toresults");
...
document.addEventListener("keydown", (evt) => {
    if(evt.key == "ArrowUp"){
        ...
        localStorage.removeItem("svd-stamps");
        localStorage.removeItem("svd-answers");
        localStorage.removeItem("svd-corrects");
        localStorage.removeItem("svd-quesnum");
        localStorage.removeItem("svd-result");
    }
});
document.addEventListener("keydown", (e) => {
    if(e.key == "ArrowDown"){
        console.log(localStorage.getItem("svd-result"));
        console.log(result);
    }
});
...
function checkHistory(){
    if(localStorage.getItem("completed")){
        isLogged = true;
        skippedQuiz = true;
        changeWrapper(buyWrapper, logsWrapper);
        checkPayment();
...
catgCards.forEach((card, idx) => {
    card.addEventListener("click", () => {
        if(isLogged){
            ...
            localStorage.setItem("svd-theme", quizTheme);
    ...
    if(currentQuestion == questionsNum){
        ...
        for(let i = 0; i < questionsNum; i++){
            if(answers[i] == corrects[i]){
                result++;
                console.log("inc: " + result + " & " + localStorage.getItem("svd-result"));
            }
        }
        checkPayment();
        setTimeout(() => {
            changeWrapper(buyWrapper, quizWrapper);
        }, 500);
    } else {
        currentQuestion++;
        changeQuestions();
...
function seeResults(){
    if(codeInputs[0].value == "5" && codeInputs[1].value == "9" && codeInputs[2].value == "4" && codeInputs[3].value == "2"){
        calculateResults();
        changeWrapper(resultWrapper, buyWrapper);
...
function pageDown(){
    curPage++;
    if(curPage == localStorage.getItem("svd-pages")){
...
document.querySelectorAll(".exit-icon").forEach((icon, idx) => {
    icon.addEventListener("click", () => {
        if(idx == 0){
            ...
            indexWrappers.forEach(wrapper => {
                wrapper.style.backgroundColor = "white";
                wrapper.querySelector("i").style.color = "black";
            });
            statContainers.forEach((cont, idx) => {
                if(idx != 0){
                    cont.style.display = "none";
...
function logServer(){
    let username = logsInputs[0].value;

    fetch('https://plucky-broad-derby.glitch.me/send', {
        method: 'POST',
        headers: {
...
async function checkPayment(){
    if(!skippedQuiz){
        localStorage.setItem("svd-stamps", JSON.stringify(timeStamps));
        localStorage.setItem("svd-answers", JSON.stringify(answers));
        localStorage.setItem("svd-corrects", JSON.stringify(corrects));
        localStorage.setItem("svd-quesnum", questionsNum);
        localStorage.setItem("svd-result", result);
        localStorage.setItem("svd-pages", resPages);
        localStorage.setItem("svd-theme", quizTheme);
    }
    setAnswers(localStorage.getItem("svd-theme"));
    removeBoxes(localStorage.getItem("svd-quesnum") - 1);
    calculateResults();
    try{
        const response = await fetch("https://plucky-broad-derby.glitch.me/members");
        const data = await response.json();

        if(data.members.join("*").includes(localStorage.getItem("username"))){
            btnToResults.style.opacity = "1";
            btnToResults.style.cursor = "pointer";
            btnToResults.addEventListener("click", () => {
                localStorage.removeItem("completed");
                setTimeout(displayResults, 2700);
                changeWrapper(resultWrapper, buyWrapper);
            });
            document.querySelector(".price").style.opacity = "0";
            document.getElementById("buyForm").style.opacity = "0";
            setTimeout(() => {
                document.querySelector(".price").style.userSelect = "none";
                document.getElementById("buyForm").style.userSelect = "none";
            }, 300);
        }
    }catch(err){
        console.error(err);
...
function changeWrapper(wrapShow, wrapHide){
    wrapHide.style.pointerEvents = "none";
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                setTimeout(() => {
                    wrapHide.style.pointerEvents = "auto";
                }, 1000);
...
function calculateResults(){
    let comparer = 100 / localStorage.getItem("svd-quesnum");
    percentage = (comparer * localStorage.getItem("svd-result")).toFixed(0);
    for(let i = 0; i < localStorage.getItem("svd-quesnum"); i++){
        let parsedStamps = JSON.parse(localStorage.getItem('svd-stamps'));
        let parsedAnswers = JSON.parse(localStorage.getItem('svd-answers'));
        let parsedCorrects = JSON.parse(localStorage.getItem('svd-corrects'));

        document.querySelectorAll(".ttaken-txt")[i].textContent = parsedStamps[i] + "s"; 
        if(parsedAnswers[i] == parsedCorrects[i]){
            colorStats(i, "hsl(209, 99%, 43%)", parsedAnswers);
        } else {
            colorStats(i, "red", parsedAnswers);
        }
    }
}
...
function colorStats(index, value, array){
    statBoxes[index].style.borderColor = value;
    document.querySelectorAll(".stat-label")[index].style.backgroundColor = value;
    if(array[index] < 4){
        document.querySelectorAll(".indexs-flex")[index].querySelectorAll(".index-wrapper")[array[index]].style.backgroundColor = value;
        document.querySelectorAll(".indexs-flex")[index].querySelectorAll(".index-wrapper")[array[index]].querySelector("i").style.color = "white";
    }
}
